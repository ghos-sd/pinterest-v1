...
PIN_HOSTS = ("pinterest.com","www.pinterest.com","pin.it",
...
def try_html_mp4(pin_url: str) -> Tuple[Optional[str], Optional[str]]:
    """ابحث مباشرة في HTML عن روابط MP4 من v.pinimg.com."""
    try:
        html = get_html(pin_url)
        # التقط أعلى جودة لو فيه query يعمل redirect
        m = re.findall(r'https://v\.pinimg\.com/[^"\s]+?\.mp4', html)
        if m:
            # لو فيه duplicate خُد الأطول (غالبًا أعلى جودة)
...
def try_pidgets(pin_url: str) -> Tuple[Optional[str], Optional[str]]:
    """Widgets/pidgets كـ fallback (أحيانًا يرجّع صورة فقط)."""
    ...
    try:
        ...
        if img_url: return img_url, "image"
    except Exception:
        pass
    return None, None
...
def try_oembed(pin_url: str) -> Tuple[Optional[str], Optional[str]]:
    try:
        r = requests.get("https://www.pinterest.com/oembed.json",
                         params={"url": pin_url}, headers=HEADERS, timeout=HTTP_TIMEOUT)
        if r.status_code != 200:
            return None, None
        data = r.json()
...
def extract_pinterest_media(pin_url: str) -> Tuple[str, str]:
    ...
    # الأولوية الجديدة: JSON → MP4 في HTML → pidgets → oembed → meta
    for fn in (try_pws_json, try_html_mp4, try_pidgets, try_oembed, try_meta_fallback):
        u, t = fn(url)
        if u:
            return u, t
    raise ValueError("لم أعثر على فيديو/صورة في هذا الرابط. قد يكون خاص/Protected.")
...
def sniff_type_by_head(url: str) -> Optional[str]:
    ...
    if any(p.endswith(ext) for ext in (".jpg",".jpeg",".png",".webp")): return "image"
    return None
...
# ===== bot commands =====
HELP_TEXT = (
    "Hi! Send me any public Pinterest Pin link and I'll download the media (video or image) for you — no API needed.\n\n"
    "Example:\n"
    "https://www.pinterest.com/pin/123456789/\n\n"
    "Commands:\n"
    "/start — About this bot\n"
    "/help — How to use\n\n"
    "Developed & maintained by @Ghostnosd."
)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await status.edit_text(
            ...
            "Make sure the Pin is public (not from a private board/app view)."
        )

def main():
    if not BOT_TOKEN:
        raise SystemExit("Please set BOT_TOKEN environment variable with your bot token.")
